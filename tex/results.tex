...
\begin{itemize}\itemsep0pt
	\item \smug-\sct{1W} operates on a single channel assuming i.i.d. white noise at each time step.
	\item \smug-\sct{1C} operates on a single channel assuming colored noise at each time step \jovo{somebody please clarify this}.
	\item \smug-\sct{MW} operates using multiple channels and white noise.
	\item \smug-\sct{MC} operates using multiple channels and colored noise.
	\item \jovo{something about AR on the means of the waveforms.  which is that?}
	\item \sct{GMM-N} is a batch Gaussian mixture model projecting into N principal components. \jovo{is N clusters or components; make sure to do GMM-5, since GMM-4 is better than GMM-3}
	\item \sct{FMM} is a recently proposed batch method
\end{itemize}
% 
% 
The single-channel experiments were all run on channel 2 (the results were nearly identical for all channels).  The spike detections for the offline methods used a threshold of 3 times the noise standard deviation \cite{Lewicki}. (Need to do more comparisons and descriptions) and windowed at a size $P=30$ (\dec{check if $P$ matches with Vinayak notation}) \jovo{vinayak uses $L$, i believe}.  The action potential window was set to 30, and PCA was used to reduce the space to $K$=3 for the experiments. 
