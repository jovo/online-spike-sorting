...
The outputs of all neurons superimposed and discretely sampled to produce the 
...
The previous section assumed a known number of neurons $N$; in practice, our recordings are a superposition of outputs of an unknown
number of neurons. We do not wish to bound this number \emph{a priori}, moreover we expect the number of neurons observed to increase as we 
...
Since only a finite number of spikes are observed in any finite interval, only a finite number of neurons will be \emph{active}.
 Spiking activity over any finite interval will be dominated by a few of these neurons (the complexity of the
...
$\quad \Lambda(\dd \bth) = \sum_{i=1}^{\infty} \lambda_i \delta_{\bth^*_i}$.
...
modeling it as a realization of a CRM. Such a random measure has the property that for any two disjoint subsets $\varTheta_1$,  $\varTheta_2 \subseteq \Theta$, the measures $\Lambda(\varTheta_1)$ and $\Lambda(\varTheta_2)$ are independent. 
This distribution over measures is induced by distributions
over the infinite sequence of weights (the $\lambda_i$'s), and the infinite sequence of their locations (the $\bth^*_i$'s). 
...
The class of CRM is determined by the \Levy measure $\rho(\lambda)$. We choose $\rho(\lambda) = \alpha \lambda^{-1}\exp(-\lambda)$;
...
Atom $i$ of the CRM corresponds to a neuron with parameters $(\lambda_i, \theta^*_i)$. We provide the equations of the resulting model in the Supplementary
material, however it will be more convenient to rerepresent the model using the marked Poisson process of Eq.~\eqref{eq:spk}. % and \eqref{eq:spk_shape}. 
