...
%\vspace{-.1in}
\subsection{Modeling a single neuron output} \label{sec:spike}
%\vspace{-.1in}
...
kernel $\mathcal{K}$. %start by assuming the noise at each observation time is i.i.d.\ Gaussian.
...
\begin{align}
  ...
  x(t) &=   \textstyle{\sum_{l \in |\mc{T}|}} \sum_{k \in [K]} y_{lk} \mathsf{d}_k(t - \tau_{l}) + \eps_t  &\text{ where }  \eps_t &\sim \text{GP}(0,\mathcal{K}).  \label{eq:CRP_mix}  %\,  l \in [|\mc{T}|]. 
