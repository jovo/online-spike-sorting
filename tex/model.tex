...
$\mt{\mb{D}}_{k,:}=(\mt{d}_k[1], \ldots, \mt{d}_k[L])\T$. These form the rows of a ${K \times L}$ matrix $\mt{\bD}$, we call its columns
$\mt{\mb{D}}_{:,l}$. The shape of the $j^{th}$ spike is now a vector of length $L$, and for a weight vector
