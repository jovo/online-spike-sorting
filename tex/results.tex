...
We compare these variants of \smug\ to Gaussian mixture models and k-means with N components (\sct{Gmm-N} and \sct{K-N}, respectively), where \sct{N} indicates the number of components.  Finally, we include a Focus Mixture Model (\sct{Fmm}) \cite{FMM}, a recently proposed Bayesian generative model with state-of-the-art performance.  Only \smug\ methods were online as we desired to compare to the state-of-the-art \emph{batch} algorithms which use all the data. 
...
algorithms.  
(b) ROC curves demonstrating that \smug\ outperforms all competitor algorithms, regardless of threshold (\jovo{*} indicates learning $\Lambda$ from the data).\label{hc1res}}
\end{SCfigure}
\end{center}
% 
% 
...
Our \smug\ achieved a heightened sensitivity by virtue of incorporating \emph{multiple} channels (see \sct{M}\smug\ point in Fig.\ \ref{hc1res}).  
